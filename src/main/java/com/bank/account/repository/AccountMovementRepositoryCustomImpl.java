...
public class AccountMovementRepositoryCustomImpl implements AccountMovementRepositoryCustom {
    ...
    @Override
    public Flux<AccountMovement> findInRange(Instant from, Instant to) {
        Query query = Query.query(Criteria.where("occurredAt").gte(from).lt(to));
        return template.find(query, AccountMovement.class);
    }

    @Override
    public Flux<AccountMovement> findByAccountIdInRange(String accountId, Instant from, Instant to) {
